    ...
    async function convertir() {
      ...
      if (amount === "" || isNaN(amount)) {
        ...
        return;
      } //verifie dàbord si l'utilisateur a entrer une valeur ou pas,ensuite on verifie que la valeur amount est un nombre ou pas et si il est pas sa renvoie true
      //ensuite il  y as une ligne qui permet d'afficher une message d'erreur avec l'id result ensuite la conditionqui est entrain de effuctuer prend fin 

      const apiKey = "80009a3a32257adcaf6f0c55"; 
      const url = `https://v6.exchangerate-api.com/v6/${apiKey}/pair/${from}/${to}`;
      //on n'a cree une constate apikey et on n'a mis la cle l'api ensuite il y a la constante url qui contient l'url de l'api et aussi ont utilise apikey qui contient la cle de l'api et il y a aussi les variable qui contiendrons les device debut et de fin
      try{
      const response = await fetch(url); //  ici ont Va chercher les données depuis l’adresse url et attendre que la réponse arrive
